...
/*
 * Catat absensi dengan logika check-in (masuk) & check-out (pulang).
 * opts: { method, date, time(ISO), phase('in'|'out'), lateMinutes }
 * Status kembalian:
 *   'checkin'     -> baru absen masuk
 *   'checkout'    -> baru absen pulang
 *   'already_in'  -> sudah absen masuk, belum waktunya pulang (tidak dihitung)
 *   'already_out' -> sudah absen masuk & pulang
 */
async function recordAttendance(memberId, opts = {}) {
  await init();
  const pool = getPool();
  const { method = 'hand-raise', date, phase = 'in' } = opts;
  ...
  // Sudah absen masuk, tapi belum waktunya pulang -> tidak dihitung.
