...
const { WORK_START, WEEKEND_MIN_GAP_MINUTES, allowedCheckoutMinutes, isWeekend } = require('./worktime');
...
async function recordAttendance(memberId, opts = {}) {
  ...
  // Akhir pekan (overtime): boleh pulang kapan saja setelah jeda minimum.
  let allowed;
  if (isWeekend(day)) {
    const checkinMin = existing.checkinMinutes != null ? existing.checkinMinutes : WORK_START;
    allowed = checkinMin + WEEKEND_MIN_GAP_MINUTES;
  } else {
    allowed = allowedCheckoutMinutes(existing.lateMinutes || 0);
  }
